...
import { FaUser, FaSearch, FaSpinner, FaUndo } from "react-icons/fa";
...
const DeletedUsers = () => {
    ...
    const fetchDeletedUsers = async () => {
      try {
        ...
        const response = await axios.get(
          "http://localhost:8080/api/v1/getdeletedusers"
        );
  ...
  const handleRestore = async (id) => {
    try {
      await axios.put(`http://localhost:8080/api/v1/restoreuser/${id}`);
  ...
  return (
    <div className="p-6 w-full mx-auto bg-white rounded-lg shadow-lg dark:bg-dark_2">
      <div className="flex items-center mb-6">
        <FaUser className="text-3xl text-red-500 mr-3" />
                ...
                >
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button
                      ...
                      className="p-2 text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 bg-green-100 hover:bg-green-200 dark:bg-green-900 dark:hover:bg-green-800 rounded-full transition-colors duration-200"
                      title="Restore User"
                    >
